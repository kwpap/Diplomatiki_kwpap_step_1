...
find_slopes_with_one_country_with_weights <- function( year = 2015, name = "none", weights = c(1,1,1,1,1,1,1,1)){
  if(year != 0) {year_for_comparison <- year}
  df_1D <- find_slopes_with_one_country(country = name, year = year_for_comparison, weight_population = weights[1], weight_GDPpc = weights[2], weight_inflation = weights[3], weight_agriculture = weights[4], weight_industry = weights[5], weight_manufacturing = weights[6], weight_total_energy_supply = weights[7], weight_verified_emisions = weights[8])$data
  lm <- find_slopes_with_one_country(country = name, year = year_for_comparison, weight_population = weights[1], weight_GDPpc = weights[2], weight_inflation = weights[3], weight_agriculture = weights[4], weight_industry = weights[5], weight_manufacturing = weights[6], weight_total_energy_supply = weights[7], weight_verified_emisions = weights[8])$linear
  return (list( data = df_1D, linear =  lm))
}
