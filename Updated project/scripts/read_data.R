...
generate_filename <- function(year) {
  paste(
    "df_free_", year,
    ifelse(will_use_log, "_log", ""),
    ifelse(will_normalise, "_norm", ""),
    ifelse(will_use_total_energy_supply, "_tes", ""),
    ifelse(will_use_verified_emisions, "_ve", ""),
    ifelse(will_use_GDPpc, "_gdp", ""),
    ifelse(will_use_population, "_pop", ""),
    ifelse(will_use_inflation, "_inf", ""),
    ifelse(will_use_agriculture, "_agr", ""),
    ifelse(will_use_industry, "_ind", ""),
    ifelse(will_use_manufacturing, "_man", ""),
    ".csv", sep = ""
  )
}
...
read_data <- function(year = 0) {
  ...
  text_t <- generate_filename(year_for_comparison)
