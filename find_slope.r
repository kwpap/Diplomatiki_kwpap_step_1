find_slope <- function(df_data, df_free){
  if (df_free[1,1] == 50){
    df_free <- df_free[-c(1),]
  }
  ...
  mod <- lm(df_1D$"df_free_distance" ~ df_1D$"df_distance")
  return (list( data = df_1D, linear =  mod))
}

create_png_for_linear_regration <- function(df_1D, name, lm){

# Create png with the regression line
png(paste("Newscatterplot_with_regression_line_",name, ".png") , width = 1000, height = 1000)
